...
As a result of these lemmas, we immediately obtain the following corollary:

\begin{cor}
Let $\Gamma$ be a diagram of finite type. Then the group A($\Gamma$) defined in the introduction is isomorphic to the group A$_{\Gamma}$
\end{cor}
...
Let A$_{\Gamma}$ be generated by $s_{1}, \dots, s_{n}$. Then $s_{1}^{-1}, \dots, s_{n}^{-1}$ satisfy the relations (R2) and (R3) in A$_{\Gamma^{op}}$.
\end{lem}
\begin{proof}
...
\item
\begin{enumerate}[i)]
\item We have that \todo{refer to lemma}
\begin{align*}
