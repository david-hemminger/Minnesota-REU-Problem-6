...
Let $\Gamma$ be a graph of finite type and suppose there are three vertices, labeled $i,j,k$ with both $i,j$ connected to $k.$ Then mutation at $k$ on the induced subdiagram appear as in one of the following figures, either from left to right or right to left, up to switching $i$ and $j,$
...
\begin{rem}
In the above definition, the chordless cycle is ordered, which means that two cycles are considered different if they are cyclic rotations of each other.
\end{rem}
