...
\begin{theorem}\cite[Theorem 5.4]{BM13}
\begin{enumerate}
...
\indent Section \ref{sec:main_definitions} provides the necessary definitions and fundamental results from \cite{BM13} to provide motivation for our own results. For further definitions and references on the topic, we refer the reader to \cite{FZ02}. Section \ref{sec:finite-type_diagrams}  will review theory from \cite{FZ02}, \cite{FZ03} as well as review the classifications (from \cite{BM13}) of mutations of diagrams and their oriented chordless cycles. In Section \ref{sec:defn_artingroup} we define the appropriate relations for our Artin group presentations. Section \ref{sec:one_relation} specifies a relation for each case of the chordless cycles which suffices to show all the other relations in the group presentation hold. Finally, Section \todo{for example}\ref{sec:proof_of_main} will provide the proof that the Artin group defined for a diagram $\Gamma$ is invariant up to isomorphism under mutations of $\Gamma$. \\
