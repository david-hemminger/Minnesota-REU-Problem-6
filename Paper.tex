...
The rest of the paper will be devoted to building up analogous relations, defined in ~\ref{grp def} to prove a similar result in the case of Artin groups. For $\Gamma$ a diagram of finite type, we define the associated Artin Group as follows. 

\begin{defn}
The associated Artin group $A_\Gamma$ is generated by $s_i,$ where there is one $s_i$ for each vertex $i$ in $\Gamma.$ These generators are subject to the following relations
\begin{itemize}
...
\end{align*}
where subscripts are taken $\pmod d.$ We will denote this relation by r(a, a+1). 
\end{itemize}
\end{defn}
 
 We will use these group relations to prove the mutation invariance of $A_{\Gamma}$ in Section \ref{sec:proof_of_main}
...
\cite[Corollary 2.3]{BM13} Let $\Gamma$ be a graph of finite type and suppose there are three vertices, labeled $i,j,k$ with both $i,j$ connected to $k.$ Then mutation at $k$ on the induced subdiagram appear as in one of the following figures, either from left to right or right to left, up to switching $i$ and $j,$
...
\todo[inline]{directions of arrows within squares?}
...
\end{note}
\begin{defn}
\cite[Beginning of section 1.2]{artin} \todo{put reference for artin, found at the bottom, in bibliography, but I don't know how to use this bibliography format.}
...
\item[(R3)] Let $(i_0,i_1,\ldots,i_{d-1})$ be an ordered tuple such that the subgraph of $\Gamma$ on the vertices $(i_0,\ldots, i_{d-1})$ is a chordless cycle, with edges of nonzero weight from $i_k$ to $i_{k+1},$ where subscripts $k$ are taken $\pmod d.$ If $(i_0,\ldots, i_{d-1})$ is such an ordered tuple, and additionally,
\begin{enumerate}
	\item All edges in the chordless cycle are of weight 1 or 2 and the edge $i_{d-1}\rightarrow i_0$ has weight 2.
	\item All edges in the chordless cycle have weight $1.$
\end{enumerate}
...
Given the relations (R2), many of the relations in (R3) become redundant. For example,
...
so that all edges have weight 1. Then if W is a group generated by $s_{1}, \dots, s_{n}$ satifying the relations (R2) and $r(i_{a}, i_{a+1})$ for some a $\in \{1, \dots, d\}$, all of the relations in (R3) hold for C.
\end{lem}

\begin{proof}
As in Barot-Marsh, it suffices to prove that the relation r(0, 1) implies the relation r(d-1, 0). So suppose $A_{\Gamma}$ satisfies the relation r(0, 1). Then we have 
\begin{align*}
...
\draw[->]
  ...
  ;
   ...
   \end{tikzpicture}
and let A be the group with generators $s_{1}, \dots, s_{n}$ defined by $\Gamma$. Then the relations r(i, j) and r(k, i) are equivalent.
\end{lem}
...
$$s_{j}s_{k}^{-1}s_{i}s_{k}s_{j}s_{k}^{-1}s_{i}^{-1}s_{k}s_{j}^{-1}s_{k}^{-1}s_{i}^{-1}s_{k} = e.$$
\end{lem}
...
\draw[->]
  ...
  ; 
\end{tikzpicture}
and let A be the group with generators $s_{1}, \dots, s_{n}$ defined by $\Gamma$. Then the relations r(i, j) and r(k, l) are equivalent.
\end{lem}
...
Let A$_{\Gamma}$ be generated by $s_{1}, \dots, s_{n}$. Then $s_{1}^{-1}, \dots, s_{n}^{-1}$ satisfy the relations (R2) and (R3) in A$_{\Gamma^{op}}$. In particular, A$_{\Gamma}$ $\cong$ A$_{\Gamma^{op}}$.
\end{lem}
\begin{proof}
One can see that the inverse elements satisfy (R2) in A$_{\Gamma^{op}}$ by taking the inverse of both sides of the corresponding relation in A$_{\Gamma}$. To see that the elements satisfy (R3) in A$_{\Gamma^{op}}$, note that for a chordless cycle in $\Gamma$ with all weights equal to one, we have $$s_{0}^{-1}\dots s_{d-2}^{-1}s_{d-1}s_{d-2}\dots s_{0} = s_{1}^{-1}\dots s_{d-2}^{-1}s_{d-1}s_{d-2}\dots s_{1}$$ by the relation r(0,1) in (R3) in A$_{\Gamma}$. But then applying relations from (R2), we have that $$s_{0}^{-1}\dots s_{d-1}s_{d-2}s_{d-1}^{-1}\dots s_{0} = s_{1}^{-1}\dots s_{d-1}s_{d-2}s_{d-1}^{-1}\dots s_{1},$$ and since the cycle is chordless, we then have $$s_{0}^{-1}s_{d-1}\dots s_{d-3}^{-1}s_{d-2}s_{d-3}\dots s_{d-1}^{-1}s_{0} = s_{d-1}s_{1}^{-1}\dots s_{d-3}^{-1}s_{d-2}s_{d-3}\dots s_{1}s_{d-1}^{-1}.$$ Repeating this process, we find that $$s_{0}^{-1}s_{d-1}s_{d-2}\dots s_{2}s_{1}s_{2}^{-1}\dots s_{d-2}^{-1}s_{d-1}^{-1}s_{0} = s_{d-1}s_{d-2}\dots s_{2}s_{1}s_{2}^{-1}\dots s_{d-2}^{-1}s_{d-1}^{-1}.$$ But this occurs if and only if $s_{1}^{-1}, \dots, s_{n}^{-1}$ satisfies the relation r(0, d-1) in A$_{\Gamma^{op}}.$ 

For a triangle labeled as in ~\ref{2tri}, by the relation r(k, i) we have $$s_{k}s_{i}^{-1}s_{j}s_{i}s_{k}^{-1} = s_{i}^{-1}s_{j}s_{i}.$$ Hence $$s_{k}s_{j}s_{i}s_{j}^{-1}s_{k}^{-1} = s_{j}s_{i}s_{j}^{-1}.$$ But as before, this can occur if and only if $s_{i}^{-1}, s_{j}^{-1}, s_{k}^{-1}$ satisfy the relation r(k, j) in A$_{\Gamma^{op}}$.

Finally, given a square labeled as in ~\ref{2sqr} and the relations r(1, 2) and r(3, 4), we have 
\begin{align*}
...
\end{align*}
But this relation holds if and only if $s_{i}^{-1}, \dots, s_{l}^{-1}$ satisfy r(j, i) in A$_{\Gamma^{op}}$. Therefore, we are done.
\end{proof}

\section{Proof of Main Result}\label{sec:proof_of_main}


\noindent In this section we prove our main result:
...
Throughout this section we will fix a diagram of finite type $\Gamma$, a vertex $k$ of $\Gamma$, and write $\Gamma^{\prime} = \mu_k(\Gamma)$.  We will write $s_i$, $r_i$, $q_i$, and $u_i$ for the generators corresponding to vertex $i$ of $A_{\Gamma}$, $A_{\Gamma^\prime}$, $A_{\Gamma^{op}}$, and $A_{\left(\Gamma^\prime\right)^{op}}$, respectively.  We will also abuse notation and write $u_i$ for the generators of $A_{\left(\Gamma^{op}\right)^\prime}$ as well, making use of the fact that $\left(\Gamma^\prime\right)^{op} = \left(\Gamma^{op}\right)^\prime$.

In the proof of Theorem \ref{thm:main} we will use Lemma \ref{lem:op_homomorphism} along with the following proposition, whose proof we will defer until after the proof of Theorem \ref{thm:main} and then split into Lemmas \ref{lem:r2a_relations}, \ref{lem:r2b_relations}, and \ref{lem:r3_relations}.
...
\todo{do we define $m_{ij}'$ somewhere?}

\begin{proof}[Proof of Theorem \ref{thm:main}]
By Proposition \ref{prop:ti_relations} $\varphi\colon A_{\Gamma^\prime}\rightarrow A_\Gamma$ is a group homomorphism.  We also abuse notation and write $\varphi\colon A_{\Gamma^{op}}\rightarrow A_{\left(\Gamma^{op}\right)^\prime}$ by letting $\varphi(q_i)$ equal $u_ku_iu_k^{-1}$ if there is an arrow $i\rightarrow k$ in $\left(\Gamma^{op}\right)^\prime$, and $u_i$ otherwise, and this is a homomorphism by Proposition \ref{prop:ti_relations} as well.  By Lemma \ref{lem:op_homomorphism}, the homormorphism $\Delta$ that sends $s_i\in A(\Gamma)$ to $q_i^{-1}\in A(\Gamma^{op})$ and $u_i\in A_{\left(\Gamma^\prime\right)^{op}}$ to $r_i\in A_{\Gamma^\prime}$ is well-defined.  We then have a homomorphism 
...
$$\psi\circ\varphi(r_i) = \Delta(\varphi(\Delta(\varphi(r_i)))) = \Delta(\varphi(\Delta(s_ks_is_k^{-1}))) = \Delta(\varphi(q_k^{-1}q_i^{-1}q_k)) = \Delta(u_i^{-1}) = r_i$$
...
$$\psi\circ\varphi(r_i) = \Delta(\varphi(\Delta(\varphi(r_i)))) = \Delta(\varphi(\Delta(s_i))) = \Delta(\varphi(q_i^{-1})) = \Delta(u_i^{-1}) = r_i$$
...
We prove Proposition \ref{prop:ti_relations} by showing that that the elements $\varphi(r_i)\in A_\Gamma$ satisfy the $(R2^\prime)$ and $(R3^\prime)$ relations in $A_{\Gamma^\prime}$.  Proofs that the $t_i$ satisfy these relations are divided among Lemmas \ref{lem:r2a_relations}, \ref{lem:r2b_relations}, and \ref{lem:r3_relations}.  Throughout the proofs we write $t_i = \varphi(r_i)$ for convenience.


\begin{lem}\label{lem:r2a_relations}
Let $i,j$ be distinct vertices of $\Gamma$.
\begin{enumerate}[(a)]
\item If $i=k$ or $j=k$, then $\langle t_it_j \rangle^k = e$.
\item If at most of $i,j$ is connected to $k$ in $\Gamma$, then $\langle t_it_j \rangle^k = e$.
\end{enumerate}
\end{lem}
...
\noindent
{\sf Case $m_{ij} = 3$}.
Here we have $s_js_is_j = s_is_js_i$, so $s_is_j = s_js_is_js_i^{-1}$, so

$$t_it_jt_i = s_is_is_js_i^{-1}s_i = s_is_is_j = s_is_js_is_js_i^{-1} = t_jt_it_j$$

\vskip.1in
\noindent
{\sf Case $m_{ij} = 4$}.  Here we have $s_is_js_is_j = s_js_is_js_i$, so $s_is_is_js_is_js_i^{-1} = s_is_js_is_j$ and therefore

$$t_it_jt_it_j = s_is_is_js_i^{-1}s_is_is_js_i^{-1} = s_is_is_js_is_js_i^{-1} = s_is_js_is_j = s_is_js_i^{-1}s_is_is_js_i^{-1}s_i = t_jt_it_jt_i$$

\vskip.1in
\noindent
{\sf Case $m_{ij} = 6$}.  Here we have $s_is_is_js_is_js_is_js_i^{-1} = s_is_js_is_js_is_j$.  As in the previous case, we add an remove pairs $s_is_i^{-1}$ as necessary, giving
...
\vskip.1in
\noindent
{\sf Case $m_{ij} = 2$}.  Here $s_is_j = s_js_i$, so we have that

$$t_it_j = s_ks_is_k^{-1}s_j = s_js_ks_is_k^{-1} = t_jt_i$$

\vskip.1in
\noindent
{\sf Case $m_{ij} = 3$}.  Here $s_is_js_i = s_js_is_j$, so we have that 

$$t_it_jt_i = s_ks_is_k^{-1}s_js_ks_is_k^{-1} = s_ks_is_js_is_k^{-1} = s_ks_js_is_js_k^{-1} = s_js_ks_is_k^{-1}s_j = t_jt_it_j$$

\vskip.1in
\noindent
{\sf Case $m_{ij} = 4$}.  Here $s_is_js_is_j = s_js_is_js_i$, so we have that

$$t_it_jt_it_j = s_ks_is_k^{-1}s_js_ks_is_k^{-1}s_j = s_ks_is_js_is_js_k^{-1} = s_ks_js_is_js_is_k^{-1} = s_js_ks_is_k^{-1}s_js_ks_is_k^{-1} = t_jt_it_jt_i$$

\vskip.1in
\noindent
{\sf Case $m_{ij} = 6$}.  Here $s_is_js_is_js_is_j = s_js_is_js_is_js_i$, so we have that
...
Let $i,j$ be distinct vertices of $\Gamma$ such that $i$ and $j$ are connected.  Then $\langle t_it_j \rangle^k = e$.
\end{lem}

\begin{proof}
The possibilities for the subdiagram induced by $i$, $j$, and $k$ are enumerated in Corollary \ref{local_three_picture}.  We show that $t_i$ and $t_j$ satisfy the $(R2^\prime)$ relations by checking each case.  We also check that the corresponding $(R3^\prime)$ relations hold in cases (b), (d), (e), and (f).  Within each case we view the subdiagram on the left as a subdiagram of $\Gamma$ in the subcase $(i)$ and as a subdiagram of $\Gamma^\prime$ in the subcase $(ii)$.\todo{Disaster.  Open to suggestions for how to make this more clear}
...
\item
\begin{enumerate}[i)]
\item We have $t_it_j = s_ks_is_k^{-1}s_ks_js_k^{-1} = s_ks_is_js_k^{-1} = s_ks_js_is_k^{-1} = t_jt_i$.
\item We have $t_it_j = s_is_j = s_js_i = t_jt_i$.
\end{enumerate}
\item
\begin{enumerate}[i)]
\item We have
\begin{align*}
t_it_jt_i &= s_ks_i\underline{s_k^{-1}s_js_k}s_is_k^{-1}\\
...
\item We have $t_it_j = s_i\underline{s_ks_js_k^{-1}} = \underline{s_is_j^{-1}s_ks_j} = \underline{s_j^{-1}s_ks_j}s_i = s_ks_js_k^{-1}s_i = t_jt_i$
\end{enumerate}
\item
\begin{enumerate}[i)]
\item We have $t_it_j = s_ks_is_k^{-1}s_ks_js_k^{-1} = s_ks_is_js_k^{-1} = s_ks_js_is_k^{-1} = t_jt_i$
...
\item We have
\begin{align*}
t_it_jt_it_jt_i^{-1}t_j^{-1}t_i^{-1}t_j^{-1} &= s_ks_is_k^{-1}s_js_ks_i\underline{s_k^{-1}s_js_k}s_i^{-1}\underline{s_k^{-1}s_j^{-1}s_k}s_i^{-1}s_k^{-1}s_j^{-1}\\
...
\item We have $t_it_j = s_is_ks_js_k^{-1} = s_is_j^{-1}s_ks_j = s_j^{-1}s_ks_js_i = s_ks_js_k^{-1}s_i = t_jt_i$
\end{enumerate}
\item
\begin{enumerate}[i)]
\item We have
\begin{align*}
t_it_jt_it_jt_i^{-1}t_j^{-1}t_i^{-1}t_j^{-1} &= \underline{s_ks_is_k^{-1}}s_j\underline{s_ks_is_k^{-1}}s_j\underline{s_ks_i^{-1}s_k^{-1}}s_j^{-1}\underline{s_ks_i^{-1}s_k^{-1}}s_j^{-1}\\
...
$$t_it_jt_i^{-1}t_j^{-1} = s_is_k\underline{s_js_k^{-1}s_i^{-1}s_k}s_j^{-1}s_k^{-1} = s_is_ks_k^{-1}s_i^{-1}s_ks_js_j^{-1}s_k^{-1} = e$$
...
\item We have that
\begin{align*}
s_k^{-1}t_it_jt_it_j^{-1}t_i^{-1}t_j^{-1}s_k &= s_is_k^{-1}s_js_ks_is_k^{-1}s_j^{-1}s_ks_i^{-1}s_k^{-1}s_j^{-1}s_k\\
&= e
\end{align*}

Where the second equality follows from Lemma \ref{lem:extra_square_relation}.
...
\begin{proof}
We know that every chordless cycle in $\Gamma'$ arises from a subdiagram of $\Gamma$ in the form of one of the cases of Lemma \ref{lem:chordless_cycles}, so we simply need to check that a cycle relation holds in each case.  We follow the labeling of the vertices used in Lemma \ref{lem:chordless_cycles}.

\begin{enumerate}[a)]
\item We have $t_1t_2^{-1}t_3t_2 = s_1s_2^{-1}s_2s_3s_2^{-1}s_2 = s_1s_3 = s_3s_1 = t_2^{-1}t_3t_2t_1$

\item We have $ t_1t_2^{-1}t_3t_2 = s_1s_2^{-1}s_2s_3s_2^{-1}s_2 = s_1s_3 = s_3s_1 = t_3t_1$

\item We have $t_1t_2^{-1}t_3t_2 = s_1s_2^{-1}s_2s_3s_2^{-1}s_2 = s_3s_1 = t_2^{-1}t_3t_2t_1$

\item We have
\begin{align*}
t_3t_1^{-1}t_2t_1t_3^{-1}t_1^{-1}s_2^{-1}t_1 &= s_2s_3s_2^{-1}s_1^{-1}s_2s_1s_2s_3^{-1}s_2^{-1}s_1^{-1}s_2^{-1}s_1\\
&= s_2s_3s_1s_2s_1^{-1}s_2^{-1}s_2s_3^{-1}s_2^{-1}s_1^{-1}s_2^{-1}s_1\\
&= s_2s_3s_1s_2s_1^{-1}s_3^{-1}s_2^{-1}s_1^{-1}s_2^{-1}s_1\\
&= s_2s_3s_1s_2s_1^{-1}s_3^{-1}s_1s_2^{-1}s_1^{-1}s_2^{-1}\\
&= s_2s_1s_2s_1^{-1}s_3s_3^{-1}s_1s_2^{-1}s_1^{-1}s_2\\
&= e\\
\end{align*}
...
&= s_2^{-1}2s_3^{-1}s_2^{-1}s_4s_2s_3s_2^{-1}s_2s_1\\
&= t_2^{-1}t_3^{-1}t_4t_3t_2t_1\\
\end{align*}
...
&= s_4^{-1}\underline{s_1^{-1}s_2s_1}s_4s_3\\
